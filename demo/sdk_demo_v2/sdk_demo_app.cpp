...
void CSDKDemoApp::GetCustomUIHWND(HWND *w1, HWND *w2) {
	if (nullptr != m_customizedMeetingUI->GetHWND()) {
		*w1 = m_customizedMeetingUI->GetHWND();
		*w2 = m_customizedMeetingUI_->GetHWND();
	}
...
void CSDKDemoApp::InitCustomizedUI()
{
	if (SDKInterfaceWrap::GetInst().IsSelectCustomizedUIMode())
	{
		if(NULL == m_customizedMeetingUI)
		{
			...
			m_customizedMeetingUI_ = new CCustomizeInMeetingUIMgr();
			
			m_customizedMeetingUI_->SetEvent(this);
			if(m_customizedMeetingUI)
